<template>
  <div class="app-container">
    ...
    <el-table :data="serviceConfigData" border style="width: 100%; margin-top: 20px">
      <el-table-column prop="serviceType" label="服务类型"></el-table-column>
      <el-table-column label="服务商">
        <template #default="scope">
          <div v-if="Array.isArray(scope.row.selectedProviders)">
            <div v-for="(provider, index) in scope.row.selectedProviders" :key="index">
              {{ provider }}
            </div>
          </div>
          <div v-else>
      ...
      <el-table-column label="操作" width="180">
        <template #default="{ row }">
          ...
          <el-button type="danger" link @click="deleteRow(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
